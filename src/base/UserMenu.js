...
async function UserMenu(username, faceUrl) {
    ...
    (0, terminal_kit_1.terminal)(`\n\n`);
    terminal_kit_1.terminal.yellow.bgBlack('Select the Option: ');
    terminal_kit_1.terminal.inputField({
        ...
        if (input === "1") {
            ...
            await new Promise(resolve => setTimeout(resolve, 5000));
            loading.stop();
            const response = await (await axios_1.default.request(options)).data;
            if (response.data && response.data.items && Array.isArray(response.data.items)) {
                response.data.items.forEach(async (follower) => {
                    ...
                    try {
                        const body = await axios_1.default.get(follower.profile_pic_url, { responseType: 'arraybuffer' });
                        (0, terminal_kit_1.terminal)(`\n\n`);
                        await (0, terminal_img_1.draw)(body.data, { height: 30, width: 30 });
                    }
                    catch (err) {
                        console.log("Err");
                    }
                ...
                (0, MessageLog_1.messageLog)(MessageLog_1.MessageType.ERROR, "SOMETHING WENT WRONG !!!");
                process.exit();
            }
            terminal_kit_1.terminal.white.bgBlack("InstaX By Amit Kumar Singh");
        }
        if (input === "2") {
            // Loading 
            terminal_kit_1.terminal.clear();
